...
> **最終更新**: 2026-05-05  
> *v2.5 UIリファクタリング：ドブモンUI一元化、ゲームUIサブディレクトリ化、match/パッケージ化、starter.py移動*
...
│  managers/starter.py    — ゲーム開始UIルーティング ※v2.5移動  │
└──────┬──────────────────────┬───────────────────────┘
       │ ゲームロジック         │ 経済ルール
┌──────▼──────────┐  ┌────────▼──────────────────────┐
...
### core/ui/（UI共通コンポーネント）
- **`view_base.py`** — `BaseView` / `JoinView`: `interaction_check` による認可チェックを内包した全Viewの基底クラス。エラーフォーマッターレジストリも发指。

> [!NOTE]
> v2.5 以前にここにあった `starter.py` は `managers/starter.py` に移動しました。

### managers/（セッション管理）
- **`manager.py`** — `GameManager` クラス。各チャンネルのゲームセッションの一元管理。同チャンネルでの二重起動を防止し、DBを経由したステートレス設計を実現。
- **`game_session.py`** — `BaseGameSession`。参加者管理・Pot管理・ターンローテーション・返金処理などの共通機能。
- **`starter.py`** — `execute_game_start()`: `JoinView` の「開始」ボタン押下後に呼ばれる。セッションの型（Blackjack/Chinchiro/Poker）を判別し、各ゲームのViewを生成・表示する。※v2.5で `core/ui/` から移動。
...
| `ui/` | Discord UIパッケージ。全てのViewクラス群（戦闘・購入・ショップ等） ※v2.5統合 |
| `dob_battle/` | 戦闘計算パッケージ。エンジン・計算・精算・野生戦闘 |
| `dob_shop/` | ショップシステム。アイテム定義・効果適用・サービスロジック |
| `genetics/` | 遺伝・配合計算パッケージ。メンデル型・突然変異・固定管理（MutationEngine） |
...
| **UI** | `ui/` | `view_base.py` (JoinView/BaseView 基底) |
