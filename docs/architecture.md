...
> **最終更新**: 2026-05-01  
> *v2.17 ファイル構成実態反映：core/ui再編成・dob_views追記*
...
ブラックジャック・ポーカーのカード画像は `data/card/` から読み込んで Canvas 合成する方式に統一されています。ポーカーでは背景のテーブル画像も動的に生成・キャッシュされ、静的な背景画像ファイルへの依存を排除しています。
