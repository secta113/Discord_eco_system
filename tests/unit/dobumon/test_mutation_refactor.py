...
def test_factory_undead_lifespan():
    ...
    with patch("logic.dobumon.genetics.dob_mendel.MendelEngine.resolve_traits", return_value=["undead"]):
        # 通常の初期寿命は 80-120 の間
...
def test_taboo_red_logic():
    ...
    with patch("logic.dobumon.genetics.dob_mendel.MendelEngine.resolve_traits", return_value=["forbidden_red"]):
